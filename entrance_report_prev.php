...
<body>
    <header>
        ...
        <?php 
            if($type == "all"){
                echo '<a class="print" href="exportsheet.php" style="font-size:20px;float:right;margin-right:5px;text-decoration:none;">Export to Excel</i></a>';
                echo '<h1 style="position: relative;top: 20px;left: 20px;">Entrance Report -Full Report </h1>';
            }else{
                echo '<h1 style="position: relative;top: 20px;left: 20px;">Entrance Report</h1>';
            }
    
            if(!isset($_GET['date'])){
                $dateto = htmlentities($_GET['dateto']);
                $datefrom = htmlentities($_GET['datefrom']);
                if($dateto ==""){
                    header("Location: /inventory_report.php?type=$type");
                }
                if($datefrom ==""){
                    header("Location: /inventory_report.php?type=$type");
                }
                ?>
                    <p class="date" style="position: relative;top: 20px;left: 20px;font-size:15px;">Entrance Report: <?php echo $dateto." - ".$datefrom; ?></p>
                <?php
            }else{
                ...
                if($date == "all" || $date == "All" ){
                    ?>
                        <p class="date" style="position: relative;top: 20px;left: 20px;font-size:15px;">Entrance Report - All </p>
                    <?php
                }else{
                    header("Location: /entrance_report.php?type=$type");
                }
            ...
            <tr>
                <?php
                    ...
                    if(!isset($_GET['date'])){
                        ...
                        $start_date_formatted = date('Y-m-d', strtotime($datefrom));
                        $end_date_formatted = date('Y-m-d', strtotime($dateto));
                        if($type == "all" || $type =="All"){
                        $sql = "SELECT * 
                        FROM entrance e 
                        INNER JOIN members m ON e.MemberID = m.MemberID 
                        WHERE m.Deleted = 0 AND DateAdded BETWEEN ? AND ? GROUP BY m.MemberID";
                        ...
                        FROM entrance e 
                        INNER JOIN members m ON e.MemberID = m.MemberID 
                        WHERE m.TypeId  = 1 AND m.Deleted = 0 AND DateAdded BETWEEN ? AND ? ";
                        $stmt = $conn->prepare($sql);
                        $stmt->bind_param("ss", $start_date_formatted, $end_date_formatted);
                            ...
                            FROM entrance e 
                            INNER JOIN members m ON e.MemberID = m.MemberID 
                            WHERE m.TypeId  = 3 AND m.Deleted = 0 AND DateAdded BETWEEN ? AND ? D";
                            $stmt = $conn->prepare($sql);
                            $stmt->bind_param("ss", $start_date_formatted, $end_date_formatted);
                    }else if($type =="Staff"){
                            $sql = "SELECT * 
                            FROM entrance e 
                            INNER JOIN members m ON e.MemberID = m.MemberID 
                            WHERE m.TypeId  = 4 AND m.Deleted = 0 AND DateAdded BETWEEN ? AND ? ";
                            $stmt = $conn->prepare($sql);
                            $stmt->bind_param("ss", $start_date_formatted, $end_date_formatted);
                    }else if($type =="Graduate-School"){
                            $sql = "SELECT * 
                            FROM entrance e 
                            INNER JOIN members m ON e.MemberID = m.MemberID 
                            WHERE m.TypeId  = 2 AND m.Deleted = 0 AND DateAdded BETWEEN ? AND ? ";
                            $stmt = $conn->prepare($sql);
                            $stmt->bind_param("ss", $start_date_formatted, $end_date_formatted);
                    }else{
                        $sql = "SELECT * 
                        ...
                        if ($resultCheck > 0) {
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                echo "<tr>";
                                echo "<td>" . htmlentities($row['MemberID']) . "</td>";
                                echo "<td>" . htmlentities($firstname) . " " .  htmlentities($lastname). "</td>";
                                echo "<td>" .  htmlentities($course). "</td>";
                                echo "<td>" .  htmlentities($formattedDate). "</td>";
                                echo "<td>" .  htmlentities($types) . "</td>";
                                echo "</tr>";
                        ...
                        if($type =="all"){
                            ...
                            FROM entrance e 
                            INNER JOIN members m ON e.MemberID = m.MemberID 
                            WHERE m.Deleted = 0 ORDER BY DateAdded DESC";
                        }else if($type =="Student"){
                            $sql = "SELECT * 
                            FROM entrance e 
                            INNER JOIN members m ON e.MemberID = m.MemberID 
                            WHERE m.TypeId  = 1 AND m.Deleted = 0 ORDER BY DateAdded DESC";
                        }else if($type =="Faculty"){
                            $sql = "SELECT * 
                            FROM entrance e 
                            INNER JOIN members m ON e.MemberID = m.MemberID 
                            WHERE m.TypeId  = 3 AND m.Deleted = 0 ORDER BY DateAdded DESC";
                        }else if($type =="Staff"){
                            $sql = "SELECT * 
                            FROM entrance e 
                            INNER JOIN members m ON e.MemberID = m.MemberID 
                            WHERE m.TypeId  = 4 AND m.Deleted = 0 ORDER BY DateAdded DESC";
                        }else if($type =="Graduate-School"){
                            $sql = "SELECT * 
                            FROM entrance e 
                            INNER JOIN members m ON e.MemberID = m.MemberID 
                            WHERE m.TypeId  = 2 AND m.Deleted = 0 ORDER BY DateAdded DESC";
                        }
