...
<body>
    ...
    <header>
        ...
        <?php if($type == "all"){
            echo '<h1 style="position: relative;top: 20px;left: 20px;">Top Borrowers -All </h1>';
        }else{
            if($type == "o"){
                echo '<h1 style="position: relative;top: 20px;left: 20px;">Top Borrowers - Overnight</h1>';

            }else if($type =="r"){
                echo '<h1 style="position: relative;top: 20px;left: 20px;">Top Borrowers -Research</h1>';
            }else if($type =="p"){
                echo '<h1 style="position: relative;top: 20px;left: 20px;">Top Borrowers -Photocopy/h1>';
            }else{
        ...
        if(isset($_GET['datefrom']) && isset($_GET['dateto'])){
            echo '<p style="position: relative;top: 20px;left: 20px;">Date From: '.$datefrom.' - '.$dateto.'</p>';
        }else{
            header('Location: /topborrower_form.php');
        }
        ?>
    ...
    <table>
        ...
        <tr>
            ...
            if($type == "all"){
                    ...
                    $sql = "SELECT m.MemberID, CONCAT(m.Firstname, ' ', m.Lastname) AS MemberName, COUNT(b.MemberID) AS Total 
                    FROM `borrowed` b 
                    INNER JOIN `members` m ON b.MemberID = m.MemberID 
                    WHERE b.MemberID = m.MemberID AND DATE(DateBorrowed) BETWEEN ? AND ? AND b.Return = '1' 
                    GROUP BY MemberName 
                    ORDER BY Total DESC";
                        ...
                        if ($resultCheck > 0) {
                            while ($row2 = mysqli_fetch_assoc($result2)) {
                                ...
                                echo "<tr>";
                                echo "<td>" . htmlentities($row2['MemberID']) . "</td>";
                                echo "<td>" . htmlentities($row2['MemberName'])."</td>";
                                echo "<td>" . htmlentities($row2['Total']) . "</td>";
                                echo "</tr>";
                        ...
                        FROM `borrowed` b 
                        INNER JOIN `members` m ON b.MemberID = m.MemberID 
                        WHERE b.MemberID = m.MemberID AND DATE(DateBorrowed) BETWEEN ? AND ? AND b.Return = '1' AND b.Purpose ='Overnight'
                        GROUP BY MemberName 
                        ORDER BY Total DESC";
                            $stmt = $conn->prepare($sql);
                            $stmt->bind_param("ss", $datefrom, $dateto);
                        ...
                        FROM `borrowed` b 
                        INNER JOIN `members` m ON b.MemberID = m.MemberID 
                        WHERE b.MemberID = m.MemberID AND DATE(DateBorrowed) BETWEEN ? AND ? AND b.Return = '1' AND b.Purpose ='PhotoCopy'
                        GROUP BY MemberName 
                        ORDER BY Total DESC";
                            $stmt = $conn->prepare($sql);
                            $stmt->bind_param("ss", $datefrom, $dateto);
