<?php
    ...
    function isValidYear($year) {
        $currentYear = date('Y');
        return is_numeric($year) && strlen($year) == 4 && $year;
    }
    if (empty($year) || empty($type || $statu)) {
        // One or more variables are empty
        // Handle the error here
        header('Location: /inventory_report.php');
        exit;
    } else {
        if($type !="Books" && $type !="Thesis"){
            header('Location: /inventory_report.php');
            exit;
        }
        if($status !="Existing" && $status !="Non-Existing"){
            header('Location: /inventory_report.php');
            exit;
        }
        if(!isValidYear($year)){
...
<body>
    <header>
        ...
        <?php
            ...
            if ($type == "Books") {
                echo '<h1 style="position: relative;top: 20px;left: 20px;font-weight:bold;">Books - ' . $status . '</h1>';
            } else if ($type == "Thesis") {
                echo '<h1 style="position: relative;top: 20px;left: 20px;font-weight:bold;">Thesis - ' . $status . '</h1>';
            } else {
                header('Location: /inventory_report.php');
            }
            ...
            <tr>
                <?php
                    ...
                    if (!empty($_GET['status'])) {
                        ...
                        if ($status == "Existing") {
                            if($type =="Books"){
                                $sql = "SELECT * FROM `inventory` WHERE `Existing` =1 AND Type ='Books' AND `Year` = ?";
                            }else{
                                $sql = "SELECT * FROM `inventory` WHERE `Existing` =1 AND Type ='Thesis' AND `Year` = ?";
                            }

                        } else if ($status == "Non-Existing") {
                            if($type =="Books"){
                                $sql = "SELECT * FROM `inventory` WHERE (`Existing` = 0 OR `Existing` = 2) AND Type ='Books' AND `Year` = ?";
                            }else{
                                $sql = "SELECT * FROM `inventory` WHERE (`Existing` = 0 OR `Existing` = 2) AND Type ='Thesis' AND `Year` = ? ";
                    ...
                    while ($row = $result->fetch_assoc()) {
                        if($type =="Books"){
                            $bookaccession = "SELECT * FROM `books accession` WHERE `AccID` = ?";
                            
                        $bookaccessionstmt = $conn->prepare($bookaccession);
                        $bookaccessionstmt->bind_param("s",$row['AccID']);
                        $bookaccessionstmt->execute();
                        $bookaccessionresult = $bookaccessionstmt->get_result();
                        while ($bookaccessionrow = $bookaccessionresult->fetch_assoc()) {



                            $subtablerow = "SELECT * FROM `books sub table` WHERE IDNo = ?";
                            $stmt2 = $conn->prepare($subtablerow);
                            $stmt2->bind_param("s", $bookaccessionrow['IDNo']);
                            $stmt2->execute();
                            ...
                            while ($row2 = $result2->fetch_assoc()) {
                                    ...
                                    $sql3 = "SELECT * FROM `books` WHERE BookID = ? AND Deleted = 0";
                        
                                $stmt3 = $conn->prepare($sql3);
                                $stmt3->bind_param("i", $row2['BookID']);
                                $stmt3->execute();
                                ...
                                while ($row3 = $result3->fetch_assoc()) {
                                    echo "<tr>";
                                    echo "<td>" . htmlentities($bookaccessionrow['AccessionNo']) . "</td>";
                                    ...
                                    echo "<td>" . htmlentities($bookaccessionrow['MR Page'])."</td>";
                            ...
                            $string = $row['AccID'];
                            $modifiedString = str_replace('.thesis', '', $string);
                            $thesisid = $modifiedString;
                            $bookaccession = "SELECT * FROM `thesis` WHERE `AccessionNo` = ? AND Type = 'Thesis'";
                            
                            $bookaccessionstmt = $conn->prepare($bookaccession);
                            $bookaccessionstmt->bind_param("s",$thesisid);
                            $bookaccessionstmt->execute();
                            $bookaccessionresult = $bookaccessionstmt->get_result();
                            while ($bookaccessionrow = $bookaccessionresult->fetch_assoc()) {
                                echo "<tr>";
                                echo "<td>" . htmlentities($bookaccessionrow['AccessionNo']) . "</td>";
                                echo "<td>" . htmlentities($bookaccessionrow['Title']) . "</td>";
                                echo "<td>" . htmlentities($bookaccessionrow['Author']). "</td>";
                                echo "<td>" . htmlentities($bookaccessionrow['CopyrightYear'])."</td>";
                                echo "<td>" . htmlentities($bookaccessionrow['MR Page'])."</td>";
