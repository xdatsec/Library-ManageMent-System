<?php
    ...
    $callnumst = htmlentities($_GET['callnumst']);
    $callnumend = htmlentities($_GET['callnumend']);
    $cat = htmlentities($_GET['category']);
    $cat2 = htmlentities($_GET['category2']);

    if (empty($start) || empty($end) || empty($subject) || empty($callnumst) || empty($callnumend) || empty($cat) || empty($cat2)) {
 
        header('Location: /call_number_report.php');
    } else {
        $subjectarray = array(); 
       
        $subjectarray[] = 'GENERALITIES';
        $subjectarray[] = 'PHILOSOPHY';
        $subjectarray[] = 'RELIGION';
        $subjectarray[] = 'SOCIAL SCIENCES';
        $subjectarray[] = 'LANGUAGES';
        $subjectarray[] = 'PURE SCIENCES';
        $subjectarray[] = 'APPLIED SCIENCES';
        $subjectarray[] = 'ARTS AND RECREATION';
        $subjectarray[] = 'LITERATURE';
        $subjectarray[] = 'HISTORY';
        $subjectarray[] = 'FICTION';
        $subjectarray[] = 'BIOGRAPHY';
        ...
        <?php
            ...
            if (!empty($_GET['location'])) {
                ...
                if ($location == "CY") {
                    ...
                    $sql = "SELECT * FROM `books accession` WHERE Location ='CY'";
                } else if ($location == "RB") {
                    $Location_Label1 = "Location - Reserved";
                    $sql = "SELECT * FROM `books accession` WHERE Location ='RB'";
                } else if ($location == "REF") {
                    $Location_Label1 = "Location - Reference";
                    $sql = "SELECT * FROM `books accession` WHERE Location ='REF'";
                } else if ($location == "Location - GS") {
                    $Location_Label1 = "Graduate School";
                    $sql = "SELECT * FROM `books accession` WHERE Location ='GS'";
                } else if ($location == "Location - FR") {
                    $Location_Label1 = "Filipianana";
                    $sql = "SELECT * FROM `books accession` WHERE Location ='FR'";
                } else {
                    header('Location: /call_number_report.php');
                }
            }
            ...
            if ($cat == "vol") {
                echo '<h1 style="position: relative;top: 20px;left: 20px;font-weight:bold;">Call Number Report from Volume</h1>';
            } else if ($cat == "title") {
                echo '<h1 style="position: relative;top: 20px;left: 20px;font-weight:bold;">Call Number Report from Title</h1>';
            } else {
                echo '<h1 style="position: relative;top: 20px;left: 20px;font-weight:bold;">Call Number Report from Volume</h1>';
            }
            $datestr = "";
            if ($cat2 == "d") {
                $datestr = "Date Received";
            } else {
                $datestr = "Copyright Year";
            }
            echo "<p style='position: relative;top: 20px;left: 20px;font-weight:bold;'> Call # from " . $callnumst . " To " . $callnumend . " and " . $datestr . " " . $start . " to " . $end . "," . $Location_Label1 . "-" . $subject . "</p>";
        ...
        <tbody>
            ...
            <tr>
                <?php
                    ...
                    $sql3 = "SELECT * FROM `books` WHERE CallNum1 BETWEEN ? AND ? AND Deleted = 0";
                    $stmt3 = $conn->prepare($sql3);
                    $stmt3->bind_param("ss", $callnumst, $callnumend);
                    $stmt3->execute();
                    ...
                    while ($row3 = $result3->fetch_assoc()) {
                        ...
                        if ($cat2 == "d") {
                            ...
                            $subtablerow = "SELECT * FROM `books sub table` WHERE BookID  = ? AND CopyrightYear BETWEEN ? AND ?";
                            $stmt2 = $conn->prepare($subtablerow);
                            $stmt2->bind_param("sss", $row3['BookID'], $startyear, $endyear);
                        }
                        ...
                        while ($row2 = $result2->fetch_assoc()) {
                            if ($cat == "vol") {
                                ...
                                if (!empty($_GET['location'])) {
                                    $location = htmlentities($_GET['location']);
                                    if ($location == "CY") {
                                        $sql = "SELECT * FROM `books accession` WHERE Location ='CY' AND IDNo = ?";
                                        $stmt = $conn->prepare($sql);
                                        $stmt->bind_param("s", $row2['IDNo']);
                                    } else if ($location == "RB") {
                                        $sql = "SELECT * FROM `books accession` WHERE Location ='RB' AND IDNo = ?";
                                        $stmt = $conn->prepare($sql);
                                        $stmt->bind_param("s", $row2['IDNo']);
                                    } else if ($location == "REF") {
                                        $sql = "SELECT * FROM `books accession` WHERE Location ='REF' AND IDNo = ?";
                                        $stmt = $conn->prepare($sql);
                                        $stmt->bind_param("s", $row2['IDNo']);
                                    } else if ($location == "GS") {
                                        $sql = "SELECT * FROM `books accession` WHERE Location ='GS' AND IDNo = ?";
                                        $stmt = $conn->prepare($sql);
                                        $stmt->bind_param("s", $row2['IDNo']);
                                    } else if ($location == "FR") {
                                        $sql = "SELECT * FROM `books accession` WHERE Location ='FR' AND IDNo = ?";
                                        $stmt = $conn->prepare($sql);
                                        $stmt->bind_param("s", $row2['IDNo']);
                                    } else {
                                        header('Location: /call_number_report.php');
                                    ...
                                    $sql = "SELECT * FROM `books accession`";
                                    $stmt = $conn->prepare($sql);
                                }
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    echo "<tr>";
                                    echo "<td>" . htmlentities($row['AccID']) . "</td>";
                                    echo "<td>" . htmlentities($row3['Title']) . "</td>";
                                    echo "<td>" . htmlentities($row3['Author1LN']) . ',' . htmlentities($row3['Author1FN']) . "</td>";
                                    echo "</tr>";
                                }
