...
var getMembersAll = {
  table: null, // Initialize table as null
  ...
  bindUIActions: function bindUIActions() {
    ...
    // Add other button-related code here
  },
  ...
  handleDataTables: function handleDataTables() {
    var dataTable = $('#myTable').DataTable({
      ...
      columnDefs: [{
        ...
        targets: 11,
        render: function render(data, type, row, meta) {
          return ' <span id="'+row.id+'" name="SchoolYearFrom" class="editable_num">'+row.SchoolYearFrom+ '</span>';
          }
        }, {
          targets: 12,
          render: function render(data, type, row, meta) {
            return ' <span id="'+row.id+'" name="SchoolYearTo" class="editable_num">'+row.SchoolYearTo+ '</span>';
          }
        }, {
          targets: 13,
          render: function render(data, type, row, meta) {
          ...
          render: function render(data, type, row, meta) {
            ...
            targets: 21,
            render: function render(data, type, row, meta) {
              return ' <span id="'+row.id+'" name="Remarks" class="editable">'+row.Remarks+ '</span>';
            }
          }, {
            targets: 22,
            render: function render(data, type, row, meta) {
  ...
  handleSearchRecords: function handleSearchRecords() {
    ...
    $('#table-search, #filterBy').on('keyup change focus', function (e) {
      ...
      // Clear selected rows
      if (self.table && value.length && (e.type === 'keyup' || e.type === 'change')) {
