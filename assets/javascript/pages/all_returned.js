  ...
  handleDataTables: function handleDataTables() {
    return $('#myTable').DataTable({
      ...
      columns: [
        ...
        {data:'DateReturned', className:'align-middle'},
        { data: 'TimeBorrowed',  className: 'align-middle'},
        { data: 'DueTime',  className: 'align-middle'},
        { data: 'TimeReturned',  className: 'align-middle'},
        { data: 'Purpose',  className: 'align-middle'},
        { data: 'Fine',  className: 'align-middle'},
      ...
      columnDefs: [{
        ...
        render: function(data, type, row, meta) {
          ...
          return '<span id="' + row.id + '" name="DueTime" class="">' + row.DueTime+'</span>';
        }
      },{
        ...
        render: function(data, type, row, meta) {
          ...
          return '<span id="' + row.id + '" name="Time Borrowed" class="">' + row.TimeBorrowed+'</span>';
        }
      },{
        ...
        render: function(data, type, row, meta) {
          ...
          return '<span id="' + row.id + '" name="TimeReturned" class="">' + row.TimeReturned+'</span>';
        }
      },{
        ...
        render: function(data, type, row, meta) {
          ...
          return '<span id="' + row.id + '" name="Fine" class="">₱' + parseInt(row.Fine, 10)+'</span>';
        }
      },{

        targets: 18,
        render: function(data, type, row, meta) {
          if(row.Paid == 1 && row.Fine > 0){
            return '<span id="' + row.id + '" name="paid" class="">Yes</span>';

          }else if(row.Paid == 0 && row.Fine > 0){
            return '<span id="' + row.id + '" name="paid" class="">Not Paid</span>';

          }else{
            return '<span id="' + row.id + '" name="paid" class="">No Fines</span>';
          }
              }
      }]
