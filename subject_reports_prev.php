<?php
    ...
    $start = htmlentities($_GET['begind']);
    $end = htmlentities($_GET['endd']);
    $subject = htmlentities($_GET['subject']);
    $cat2 = htmlentities($_GET['cat']);
    $cat = htmlentities($_GET['groupings']);

    if (empty($start) || empty($end) || empty($subject) || empty($cat) || empty($cat2)) {
        header('Location: /subject_reports.php');
    }
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <header>
        ...
        <?php
            ...
            if (!empty($_GET['subject'])) {
                if ($_GET['subject'] == "all") {
                    echo '<h1 style="position: relative;top: 20px;left: 20px;font-weight:bold;">All Subject</h1>';
                } else {
                    $getsubject = "SELECT * FROM `subject` WHERE SubjectID  = ?";
                    $stmt = $conn->prepare($getsubject);
                    $stmt->bind_param("s", $_GET['subject']);
                    $stmt->execute();
                    $result = $stmt->get_result();
                    while ($row = $result->fetch_assoc()) {
                        echo '<h1 style="position: relative;top: 20px;left: 20px;font-weight:bold;">Subject -' . $row['Subject'] . '</h1>';
                    }
                }
            } else {
                header('Location: /subject_reports.php');
            ...
            echo "<p style='position: relative;top: 20px;left: 20px;font-weight:bold;'>" . $datestr . " " . $start . " to " . $end . " " . $titlefrom . "</p>";
        ?>
            ...
            <tr>
                ...
                if (!empty($_GET['subject'])) {
                    if ($_GET['subject'] == "all") {
                        ...
                        $sql3 = "SELECT * FROM `books` WHERE Deleted = 0 AND SubjectID  = ?";
                        $stmt3 = $conn->prepare($sql3);
                        $stmt3->bind_param("s", $_GET['subject']);
                    }
                } else {
                ...
                while ($row3 = $result3->fetch_assoc()) {
                    ...
                    if ($cat == "vol") {
                        $sql = "";
                        $sql = "SELECT * FROM `books accession` WHERE IDNo = ? ORDER BY AccessionNo ASC";
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td>" . htmlentities($row3['CallNum1']) . "</td>";
                            echo "<td>" . htmlentities($row3['CallNum2']) . "</td>";
                        ...
                        $sql = "";
                        $sql = "SELECT * FROM `books accession` WHERE IDNo = ? ORDER BY ItemNo ";
                        $stmt = $conn->prepare($sql);
                        $stmt->bind_param("s", $row2['IDNo']);
                        $stmt->execute();
                        $result = $stmt->get_result();
